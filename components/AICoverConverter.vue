          ...
          <div v-for="model in currentCatModels" :key="model.modelid" :data-model-id="model.modelid"
            ...
            <div class="flex items-center">
              <button @click.stop="playAudioPreview(model, $event)"
                ...
                :aria-label="isPlayingPreview(model.modelid) ? $t('ai_cover.converter.buttons.stop') : $t('ai_cover.converter.buttons.preview')"
        ...
        <div class="bg-white p-6 border border-gray-200 rounded-b-xl flex-1">
          ...
          <div v-show="currentTab === 'cover'" class="h-full flex flex-col gap-4">
            ...
            <div class="flex-1">
              ...
              <div v-show="currentSubTab === 'upload'" class="h-full">
                <div class="h-full p-4">
                  <AudioUploader
                    ...
                    model-category="cover upload"
                    model-name="cover upload"
                    type="audio"
              ...
              <div v-show="currentSubTab === 'record'" class="h-full">
                <div class="h-full p-4">
                  <AudioRecorder
                    :recorded-file="uploadedFile"
                    model-category="cover record"
                    model-name="cover record"
                    :action-type="{ record: ActionType.RECORD_AUDIO }"
          ...
          <div v-show="currentTab === 'tts'" class="h-full flex flex-col gap-6">
            <div
              ...
              <!-- 预设文案标签 -->
              <div v-if="presets_categories.length > 0" class="flex flex-wrap gap-2 p-3 border-b border-gray-100 bg-gray-50/50 rounded-t-lg">
...
const quotaTypeForModal = ref('')//用于存储当前配额类型，传递给PayModel
...
// 使用 computed 确保 quotaType 响应式更新
...
// 计算当前分类下的模型 - 使用缓存提高性能（过滤 null/无效项，避免 v-for 或 find 时访问 .id 报错）
const currentCatModels = computed(() => {
  ...
  return options.filter(m => m != null && m.modelid != null)
})
...
watch(
  ...
  ([newDefaultCategory, newDefaultModel, newModels]) => {
    ...
    if (isInitialized.value &&
      ...
      // 深链直达时 initializeFromProps 已设好状态，仍需滚到选中项
      nextTick(() => {
        scrollSelectedModelIntoViewIfNeeded()
      })
      return
    }
...
const audioTabs = computed(() => {
  ...
  const selectedModelData = allVoiceModels.value
    ...
    .filter((m): m is Model => m != null && m.modelid != null)
...
// AudioRecorder 事件处理函数
const handleRecordSuccess = (response: any) => {
  if (response && response.url) {
    uploadedFile.value = response.file
    upresurl.value = response.url
    if (response.file) {
    ...
    // trackAction 已在 AudioRecorder 内部调用，这里不需要重复调用
  }
}

const handleRecordError = (msg: string) => {
  toast.error(msg, {
    position: 'top-right',
    duration: 3000
  })
...
const handleRecording = (val: boolean) => {
  isUploading.value = val
}

const handleFileUpload = async (event: any) => {
  const file = event.target.files[0]
  await execFileUpload(file)
}

const execFileUpload = async (file: any) => {
  if (!file) return

  if (!allowedAudioTypes.includes(file.type)) {
    const msg = t('ai_cover.converter.errors.fileFormat')
    toast.error(msg, {
      position: 'top-right',
      duration: 3000
    })
    resetUpload()
    reportError(
      new Error(`Invalid file type: ${file.type}`),
      `File upload failed - fileName: ${file.name}, pageUrl: ${pageUrl.value}`,
      uid.value,
      userEmail.value
    )
    return
  ...
  if (file.size > maxSize) {
    const msg = t('ai_cover.converter.errors.fileSize')
    toast.error(msg, {
      position: 'top-right',
      duration: 3000
    })
    resetUpload()
    reportError(
      new Error(`File size exceeds 10MB limit: ${file.size}`),
      `File upload failed - fileName: ${file.name}, pageUrl: ${pageUrl.value}`,
      uid.value,
      userEmail.value
    )
  ...
  try {
    ...
    formData.append('subscript', userSubscript.value?.toString() || '0')
    ...
    // 使用 Promise 包装 XMLHttpRequest，添加更详细的错误处理
    const response = await new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            reject({
              ret: 1,
              msg: `Invalid JSON response: ${err?.message}`,
              status: xhr.status
            })
          }
        } else {
          // 处理 502 等 HTTP 错误
          let errorMsg = `HTTP Error: ${xhr.status} ${xhr.statusText}`
          if (xhr.status === 502) {
            errorMsg = 'Server is temporarily unavailable. Please try again later.'
          } else if (xhr.status === 504) {
            errorMsg = 'Request timeout. The file may be too large.'
          }
          reject({
            ret: 1,
            msg: errorMsg,
      ...
      xhr.onerror = () => {
        // 网络错误，可能是 502 Bad Gateway
        const ret = 1
        const msg = `Network error occurred while uploading ${file.name}. This may be a server issue (502 Bad Gateway). Please try again later.`
        reject({
          ret,
          msg,
          status: xhr.status || 0
      ...
      xhr.ontimeout = () => {
        ...
        const ret = 1
        const msg = t('ai_cover.converter.errors.uploadTimeout')
      ...
      try {
        xhr.open('POST', `${apiHost}/upapi/upload`)
        xhr.send(formData)
      } catch (err) {
    ...
    if (response.ret === 0) {
      ...
      uploadedAudioList.value.length = 0
      uploadedAudioList.value.push(URL.createObjectURL(file))
      upresurl.value = response.uri
      trackAction({
      ...
      const msg = response.msg || t('ai_cover.converter.errors.uploadFailed')
      toast.error(msg, {
        position: 'top-right',
        duration: 3000
      })
      reportError(new Error(msg || 'Upload failed'), `File upload failed - msg: ${msg}, fileName: ${file.name}, pageUrl: ${pageUrl.value}`)
    }
  } catch (err: any) {
    // 处理 502 错误和其他错误
    let errorMsg = "File upload failed, please try again later, or contact us!"
    
    if (err?.status === 502 || err?.msg?.includes('502') || err?.msg?.includes('Bad Gateway')) {
      errorMsg = "Server is temporarily unavailable (502 Bad Gateway). Please try again in a few moments."
    } else if (err?.status === 504 || err?.msg?.includes('timeout')) {
    ...
    toast.error(errorMsg, {
      position: 'top-right',
      duration: 5000  // 增加显示时间，让用户看清楚错误信息
    })
    resetUpload()
    reportError(
      err,
      `File upload failed - fileName: ${file.name}, fileSize: ${file.size}, pageUrl: ${pageUrl.value}, error: ${err?.message || err?.msg || 'Unknown error'}, status: ${err?.status || 'N/A'}`,
      uid.value,
      userEmail.value
    )
  } finally {
...
const resetUpload = () => {
  ...
  // 调用 AudioRecorder 的 reset 方法
  if (audioRecorderRef.value && typeof audioRecorderRef.value.onReset === 'function') {
    audioRecorderRef.value.onReset()
  }
  uploadedFile.value = null
  uploadedFileUrl.value = null
  upresurl.value = null
...
// record audio - 现在由 AudioRecorder 组件管理
...
// 根据当前选择的模型生成预设分类（过滤 null/无效 preset，避免 evaluating 's.id' 报错）
const presets_categories = computed(() => {
  ...
  return (presets || []).filter(p => p && p.id != null).map(preset => ({
    id: preset.id,
    label: preset.label
  }))
})

// 根据当前选择的模型生成预设文本（过滤 null/无效 preset）
const presets_category_texts = computed(() => {
  const selectedModelData = currentCatModels.value.find(m => m.modelid === selectedModel.value)
  if (!selectedModelData) return []

  const presets = getCharacterPresetsForModel(selectedModelData.modelid)
  return (presets || []).filter(p => p && p.id != null).map(preset => [preset.id, preset.text])
})
...
const handleCover = async () => {
  ...
  try {
    ...
    if (result.ret === 0) {
      ...
      ret = result.ret
      msg = result.msg
      reportError(new Error(result.msg || 'Convert song failed'), `Cover song Convert failed - pageUrl: ${pageUrl.value}`, uid.value, userEmail.value)
    }
  } catch (err) {
    ret = 1
    msg = t('ai_cover.converter.errors.serverBusy')
...
const handleTTS = async () => {
  ...
  try {
    ...
    if (data?.ret === 0 && data?.uri) {
      ...
      ret = data.ret
      msg = data?.msg || t('ai_cover.converter.errors.tts.failed')
      reportError(new Error(data?.msg || t('ai_cover.converter.errors.tts.failed')), `TTS Generation failed - pageUrl: ${pageUrl.value}`)
      return [ret, msg]
    }
    return [ret, msg]
  } catch (err) {
...
const handleConvert = async () => {
  ...
  try {
    ...
    if (currentTab.value === 'cover') {
      ...
      ret = result[0]
      msg = result[1]
      // console.log("handle cover ret:", ret, "msg:", msg)
...
const playAudioPreview = async (model, event) => {
  ...
  if (!model || !model.exampleAudio || typeof model.exampleAudio !== 'string') {
    reportError(
      ...
      `Play audio failed - invalid model - pageUrl: ${pageUrl.value}`,
      uid.value,
      userEmail.value
    )
    return
  ...
  if (!model.modelid) {
    reportError(
      ...
      `Play audio failed - invalid model.modelid - pageUrl: ${pageUrl.value}`,
      uid.value,
      userEmail.value
    )
    return
    ...
    try {
      ...
      requestAnimationFrame(async () => {
        try {
          ...
          reportError(err, `Play audio failed - pageUrl: ${pageUrl.value}`, uid.value, userEmail.value)
        }
      })
    } catch (err) {
      reportError(err, `Play audio failed - pageUrl: ${pageUrl.value}`, uid.value, userEmail.value)
...
/** 列表内选中行不在可视区域时滚动（与点击选模型逻辑一致） */
const scrollSelectedModelIntoViewIfNeeded = () => {
  const element = selectedModelRef.value as HTMLElement | null
  const container = modelListContainer.value as HTMLElement | null
