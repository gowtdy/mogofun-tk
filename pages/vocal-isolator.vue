<template>
  <div class="bg-white px-6 py-8">
    ...
    <UploadPanel
      :result-title="$t('vocal_isolator.result_title')"
      :process-button-text="$t('vocal_isolator.process_button')"
      mediaType="vocal isolator"
      model-category="vocal isolator"
      model-name=""
      api-endpoint="/coverapi/vocal"
      :action-type="actionTypes"
    />
    <!-- 优势介绍组件 -->
    <AdvantagesSection
      v-if="!isLoggedIn"
      :title="t('vocal_isolator.advantages.title')"
      :advantages="advantages"
    />
    <!-- FAQ 区域 -->
    <FAQSection
      v-if="!isLoggedIn"
      :title="$t('vocal_isolator.faqs.title')"
      :faqs="faqs"
    />
  </div>
</template>
...
useHead({
  ...
  title: t('vocal_isolator.seo.title'),
  meta: [
    { name: 'description', content: t('vocal_isolator.seo.description') },
    { name: 'keywords', content: t('vocal_isolator.seo.keywords') },
    { property: 'og:type', content: 'website' },
    { property: 'og:title', content: t('vocal_isolator.seo.title') },
    { property: 'og:description', content: t('vocal_isolator.seo.description') },
    ...
    { name: 'twitter:title', content: t('vocal_isolator.seo.title') },
    { name: 'twitter:description', content: t('vocal_isolator.seo.description') },
  ...
  link: [
    ...
    { rel: 'canonical', href: `${host}/${lang.value}/vocal-isolator`},
    { rel: 'alternate', hreflang: 'x-default', href: `${host}/en/vocal-isolator` },
    { rel: 'alternate', hreflang: 'en', href: `${host}/en/vocal-isolator` },
    { rel: 'alternate', hreflang: 'zh', href: `${host}/zh/vocal-isolator` },
    { rel: 'alternate', hreflang: 'zh-tw', href: `${host}/zh-tw/vocal-isolator` },
    { rel: 'alternate', hreflang: 'es', href: `${host}/es/vocal-isolator` },
    { rel: 'alternate', hreflang: 'ja', href: `${host}/ja/vocal-isolator` },
    { rel: 'alternate', hreflang: 'fr', href: `${host}/fr/vocal-isolator` },
  ]
})

const pageKey = 'vocal-isolator'
const defaultTitle = 'Best Vocal Isolator Online'
const defaultSubtitle = 'Instantly extract vocals from a song, music, audio or video, '
...
const advantages = computed(() => {
  const advantagesItems = tm('vocal_isolator.advantages.items') || []
...
const faqs = computed(() => {
  // 先拿到原始数组，目的是获取长度
  const raw = tm('vocal_isolator.faqs.items') || []
  // 确保 raw 是数组
  const items = Array.isArray(raw) ? raw : []
  // 用 t 获取每一项的字符串
  return items.map((_, idx) => ({
    question: t(`vocal_isolator.faqs.items.${idx}.question`),
    answer: t(`vocal_isolator.faqs.items.${idx}.answer`)
  }))
})
...
const { onPageError } = usePageErrorHandler("vocal-isolator", uid.value, userEmail.value)
